...
```
{deftech}_同余闭包_在“等于”的自反、对称和传递闭包下维护术语的等价类_并且_相等的参数产生相等的函数结果的规则。
...
* {tactic}`simp` _重写_ 目标，一旦看到 {typed}`h : t₁ = t₂`，就将出现的 {lean}`t₁` 替换为 {lean}`t₂`。
  重写是定向的、破坏性的。
* {tactic}`grind` 双向_累加_相等。  没有术语被重写；相反，两位代表住在同一个班级。  所有其他引擎（{tech}[E 匹配]、理论求解器、{tech (key := "constraint propagation")}[传播]）都可以查询这些类并添加新事实，然后闭包增量更新。
