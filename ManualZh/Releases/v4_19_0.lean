...
* [#7166](https://github.com/leanprover/lean4/pull/7166) extends the notion of “fixed parameter” of a recursive function
  ...
  ```lean
  app.induct.{u_1} {α : Type u_1}（动机：列表α→列表α→Prop）（情况1：∀（bs：列表α），动机[] bs）
  ...
  ```lean
  app.induct.{u_1} {α : Type u_1}（动机：列表α→Prop）（案例1：动机[]）
...
* [#7742](https://github.com/leanprover/lean4/pull/7742) adds a feature to `structure`/`class` where binders without
  ...
  /-
  信息：CharP.cast_eq_zero_iff.{u}（R：Type u）{inst✝ : AddMonoidWithOne R}（p：Nat）[self：CharP R p]（x：Nat）：
    ↑x = 0 ↔ p ∣ x
  -/
  ```
...
* [#7482](https://github.com/leanprover/lean4/pull/7482) implements the
  ...
  theorem extractLsb'_append_eq_ite {v w} {xhi : BitVec v} {xlo : BitVec w} {start len : Nat} :
      extractLsb' 起始长度 (xhi ++ xlo) =
      如果 hstart : 开始 < w
      那么
        如果 hlen : 开始 + len < w
        然后 extractLsb' start len xlo
        否则
          (((extractLsb' (start - w) (len - (w - start)) xhi) ++
              extractLsb' start (w - start) xlo)).cast (by omega)
      否则
        extractLsb'（开始 - w）len xhi

* [#7493](https://github.com/leanprover/lean4/pull/7493) 实现 Bitwuzla 重写规则
...
* [#7508](https://github.com/leanprover/lean4/pull/7508) 显示求反与左移进行交换，即
  比特乌兹拉重写
...
* [#7594](https://github.com/leanprover/lean4/pull/7594) 实现 Bitwuzla 重写
...
* [#7595](https://github.com/leanprover/lean4/pull/7595) 实现 Bitwuzla 重写的加法重写
  [BV_EXTRACT_ADD_MUL](https://github.com/bitwuzla/bitwuzla/blob/e09c50818b798f990bd84bf61174553fef46d561/src/rewrite/rewrites_bv.cpp#L1495-L1510),
  这见证了 `i >= len` 的高位不影响
  总计 `len`：
...
* [#7757](https://github.com/leanprover/lean4/pull/7757) 为符号添加 Bitwuzla 重写 `NORM_BV_ADD_CONCAT`
  简化add-of-append。
...
* [#7398](https://github.com/leanprover/lean4/pull/7398) 修复了 cce（常见情况消除）过程中的范围界定错误
  旧代码生成器的。该通道将为
  共同的小型房舍，即使其中一些房舍位于尸体内
  本地定义的函数，这会导致范围不正确
  对连接点的引用。解决方法是在以下情况下保存/恢复候选人：
  访问 lambda。

* [#7710](https://github.com/leanprover/lean4/pull/7710) 改进了 Lean 的内存使用，特别是对于长时间运行的情况
  服务器进程，最多减少 60%
...
* [#7589](https://github.com/leanprover/lean4/pull/7589) 更改了打印机的结构实例符号，以便
  ...
  字段的值（最高可达可降低的透明度）。设置
  `pp.structureInstances.defaults` 为 true 迫使此类字段变得漂亮
  无论如何打印。
...
* [#7204](https://github.com/leanprover/lean4/pull/7204) 添加 `Id` monad 的文档字符串。

* [#7246](https://github.com/leanprover/lean4/pull/7246) 更新 Bool 的现有文档字符串并添加缺失的文档字符串。

* [#7288](https://github.com/leanprover/lean4/pull/7288) 修复了 `List.removeAll` 的文档

* [#7365](https://github.com/leanprover/lean4/pull/7365) 更新文档字符串并添加一些缺失的内容。

* [#7452](https://github.com/leanprover/lean4/pull/7452) 使出现在
  语言参考一致。

* [#7476](https://github.com/leanprover/lean4/pull/7476) 添加 `IO` 缺失的文档字符串和相关代码，并使
  与现有文档字符串的风格一致。

* [#7492](https://github.com/leanprover/lean4/pull/7492) 添加缺失的 `Array` 文档字符串并使其风格
  一致。

* [#7506](https://github.com/leanprover/lean4/pull/7506) 添加缺失的 `String` 文档字符串并生成现有文档字符串
  风格一致。

* [#7523](https://github.com/leanprover/lean4/pull/7523) 添加缺失的文档字符串并使文档字符串样式保持一致
  `System` 和 `System.FilePath`。

* [#7528](https://github.com/leanprover/lean4/pull/7528) 使 `Thunk` 的文档字符串与
  其他人。

* [#7534](https://github.com/leanprover/lean4/pull/7534) 添加缺失的 `Syntax` 相关文档字符串并使现有的
  与其他风格一致的。

* [#7535](https://github.com/leanprover/lean4/pull/7535) 修改了 `funext` 的文档字符串，使其更加简洁和
  添加对手册的参考以获取更多详细信息。

* [#7548](https://github.com/leanprover/lean4/pull/7548) 添加缺失的 monad 转换器文档字符串并使其风格
  一致。

* [#7552](https://github.com/leanprover/lean4/pull/7552) 添加了缺失的 `Nat` 文档字符串并使其风格保持一致。

* [#7564](https://github.com/leanprover/lean4/pull/7564) 更新了 `ULift` 和 `PLift` 的文档字符串，使其
  风格与其他一致。

* [#7568](https://github.com/leanprover/lean4/pull/7568) 添加缺失的 `Int` 文档字符串并制作所有文档字符串的样式
  一致。

* [#7602](https://github.com/leanprover/lean4/pull/7602) 添加 固定位宽整数 操作缺少的文档字符串
  使他们的风格保持一致。

* [#7607](https://github.com/leanprover/lean4/pull/7607) 添加 `String.drop` 和 `String.dropRight` 的文档字符串。

* [#7613](https://github.com/leanprover/lean4/pull/7613) 为出现在
  手册。

* [#7635](https://github.com/leanprover/lean4/pull/7635) 为 `Substring` 添加缺失的文档字符串，并使样式为
  `Substring` 文档字符串一致。

* [#7642](https://github.com/leanprover/lean4/pull/7642) 查看 `Float` 和 `Float32` 的文档字符串，添加缺失的内容
  并使其格式一致。

* [#7645](https://github.com/leanprover/lean4/pull/7645) 添加缺失的文档字符串并使文档字符串样式保持一致
  `ForM`、`ForIn`、`ForIn'`、`ForInStep`、`IntCast` 和 `NatCast`。

* [#7711](https://github.com/leanprover/lean4/pull/7711) 添加了手册中最后几个缺失的文档字符串。

* [#7713](https://github.com/leanprover/lean4/pull/7713) 使 BitVec 文档字符串彼此匹配以及其余部分匹配
  API 时尚。
...
* [#7178](https://github.com/leanprover/lean4/pull/7178) 修复了语言服务器中的竞争条件
  ...
  编辑文件的标题。这反过来可能会导致语义
  突出显示以停止在 VS Code 中运行，因为 VS Code 将停止
  当先前的请求被丢弃时发出请求，也会导致
  InfoView 出现缺陷。它还会导致导入自动完成
  感觉有点奇怪，因为这些请求有时会被放弃。这个
  自语言服务器首次出现以来，竞争条件就一直存在于语言服务器中
  2020 年版本。

* [#7223](https://github.com/leanprover/lean4/pull/7223) 实现并行看门狗请求处理，以便请求
  看门狗处理的数据不能阻塞主线程
  看门狗了。

* [#7240](https://github.com/leanprover/lean4/pull/7240) 添加了用于链接到语言中的部分的规范语法
  参考以及文档字符串中示例的格式根据
  文档字符串风格指南。

* [#7343](https://github.com/leanprover/lean4/pull/7343) 缓解了在 VS Code 中插入嵌入提示的问题
  双击会将镶嵌提示插入到错误的位置右侧
  编辑后。

* [#7344](https://github.com/leanprover/lean4/pull/7344) 将自动隐式嵌入提示工具提示合并为一个
  工具提示。这解决了 VS Code 中的一个问题，即 VS Code 无法
  移动时更新相邻镶嵌提示部分中工具提示的悬停
  鼠标。

* [#7346](https://github.com/leanprover/lean4/pull/7346) 修复了语言服务器遇到嵌入的问题
  删除仍处于打开状态的文件时提示断言冲突
  语言服务器。

* [#7366](https://github.com/leanprover/lean4/pull/7366) 添加了对专用“未解决的目标”的服务器端支持，并且
  “目标完成”诊断将在以下方面得到特别支持
  Lean 4 VS 代码扩展。特殊的“未解决的目标”诊断是
  改编自“未解决的目标”错误诊断，而“目标”
  当 `theorem` 或 `Prop` 类型时，发出已完成的诊断
  `example` 没有错误或 `sorry`。 Lean 4 VS 代码扩展
  同伴 PR 位于leanprover/vscode-lean4#585。

* [#7376](https://github.com/leanprover/lean4/pull/7376) 确保 `weak` 选项不必在两个 Lake 中重复
  `leanOptions` 和 `moreServerOptions`。
...
* [#7185](https://github.com/leanprover/lean4/pull/7185) 重构了 Lake 的构建内部结构，以引入
  包、模块和库之外的目标和方面。刻面，
  构建密钥、构建信息和 CLI 命令已概括为
  任意目标类型。

* [#7393](https://github.com/leanprover/lean4/pull/7393) 添加了对 Lake 配置字段的自动完成支持
  Lean DSL 位于现有字段后的缩进空格处。
  目前仍然无法在没有任何字段的情况下自动完成
  支持。

  **重大更改：** 非标准花括号配置语法现在使用分号 `;` 而不是逗号 `,` 作为分隔符。缩进仍然可以用作分隔符的替代方案。

* [#7399](https://github.com/leanprover/lean4/pull/7399) 恢复新的内置初始化器、详细说明器和宏
  Lake 回非内置。

* [#7504](https://github.com/leanprover/lean4/pull/7504) 增强了 Lake 配置数据结构声明
  （例如，`PackageConfig`、`LeanLibConfig`）以生成其他元数据
  用于自动生成 Lean 和 TOML 编码器和
  通过元程序解码器。

* [#7543](https://github.com/leanprover/lean4/pull/7543) 统一动态目标的配置声明，
  外部库、Lean 库和 Lean 可执行文件集成到单个文件中
  数据类型存储在包内的统一映射中。

  **重大更改：** 用户不能再定义具有相同名称但不同类型的多个目标（例如，Lean 可执行文件和 Lean 库均名为 `foo`）。这不会影响大多数用户，因为 Lake DSL 已经阻止了这种情况。

* [#7576](https://github.com/leanprover/lean4/pull/7576) 将 Lake 更改为在以下情况下在 Windows 上生成和使用响应文件：
  构建可执行文件和库（静态和共享）。这样做是为了
  避免可能超过 Windows 命令行长度限制。

* [#7586](https://github.com/leanprover/lean4/pull/7586) 更改 Lean 库的 `static.export` 方面以生成
  瘦静态库。

* [#7608](https://github.com/leanprover/lean4/pull/7608) 删除了 Lake 构建和中 Lake 插件的使用
  配置文件。

* [#7667](https://github.com/leanprover/lean4/pull/7667) 将 Lake 更改为记录来自 Lean 配置的消息
  它记录来自 Lean 版本的消息的方式。例如，这会删除
  冗余的严重性标题。

* [#7703](https://github.com/leanprover/lean4/pull/7703) 添加 `input_file` 和 `input_dir` 作为新的目标类型。它还
  添加了 Lean 库的 `needs` 配置选项
  可执行文件。此选项概括了 `extraDepTargets`（这将是
  将来已弃用），为声明提供更丰富的支持
  跨包和目标类型边界的依赖关系。

* [#7716](https://github.com/leanprover/lean4/pull/7716) 为 Lean 添加 `moreLinkObjs` 和 `moreLinkLibs` 选项
  包、库和可执行文件。这些用作功能性
  `extern_lib` 的替代品并提供额外的灵活性。

  **重大更改：** `precompileModules` 现在仅单独加载当前库的模块。其他库的模块通过该库的共享库一起加载。

* [#7732](https://github.com/leanprover/lean4/pull/7732) 弃用 `extraDepTargets` 并修复了由
  配置重构。

* [#7758](https://github.com/leanprover/lean4/pull/7758) 从 FFI 示例中删除 `-lstdcpp` 额外链接参数。
  实际上没有必要。

* [#7763](https://github.com/leanprover/lean4/pull/7763) 更正构建密钥获取以生成具有正确数据的作业
  类型并修复从关键文字到目标的失败强制。
...
* [#7453](https://github.com/leanprover/lean4/pull/7453) 将“(内核)”添加到内核级别应用程序的消息中
  类型不匹配错误。

* [#7769](https://github.com/leanprover/lean4/pull/7769) 修复了发布自动化脚本中的一些错误，添加了
  将标签合并到远程 `stable` 分支的脚本，并使主
  `release_checklist.py` 脚本给出调用的建议
  需要时编写 `merge_remote.py` 和 `release_steps.py` 脚本。
