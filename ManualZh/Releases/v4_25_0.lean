...
section
variable (α : Type)
共导 infSeq (r : α → α → Prop) : α → Prop 其中
  |步骤： r a b → infSeq r b → infSeq r a

/--
信息：infSeq.coinduct（α：Type）（r：α→α→Prop）（pred：α→Prop）（hyp：∀（a：α），pred a→∃b，r a b ∧ pred b）
...
/--
信息：infSeq.step (α : Type) (r : α → α → Prop) {a b : α} : r a b → infSeq α r b → infSeq α r a
-/
#guard_msgs in
#check infSeq.step
end
...
/--
信息：tick.mutual_induct (pred_1 pred_2 : Prop) (hyp_1 : pred_1 → pred_2 → False) (hyp_2 : (pred_1 → False) → pred_2) :
  (pred_1 → 勾选) ∧ (tock → pred_2)
-/
#guard_msgs in
...
example [LE α] [LT α] [Std.LawfulOrderLT α] [Std.IsLinearPreorder α] [CommRing α] [OrderedRing α]
    (a b c d : α) : a - b ≤ 5 → Ø (c ≤ b) → Ø (d ≤ c + 2) → d ≤ a - 8 → False := by
  研磨-linarith（分割：= 0）
```
...
- [#10302](https://github.com/leanprover/lean4/pull/10302) introduces the `@[method_specs]` attribute. It can be applied to
  ...
  /--
  信息: 定理 instBEqL.beq_spec_2.{u_1} : ∀ {α : Type u_1} [inst : BEq α] (x_2 : α) (xs : L α) (y : α) (ys : L α),
...
- [#10346](https://github.com/leanprover/lean4/pull/10346) lets `deriving BEq` and `deriving Ord` use `@[method_specs]`
  ...
  /--
  信息：定理 instBEqO.beq_spec_2.{u_1} : ∀ {α : Type u_1} [inst : BEq α] (a b : α), (O.some a == O.some b) = (a == b)
  -/
  #guard_msgs in #print sig instBEqO.beq_spec_2
  /--
  信息: 定理 instOrdO.compare_spec_2.{u_1} : ∀ {α : Type u_1} [inst : Ord α] (x : O α),
    (x = O.none → False) → 比较 O.none x = Ordering.lt
  -/
...
* [#10600](https://github.com/leanprover/lean4/pull/10600) implements support for negative constraints in `grind order`.
  ...
  example [LE α] [LT α] [Std.LawfulOrderLT α] [Std.IsLinearPreorder α]
      (a b c d : α) : a ≤ b → Ø (c ≤ b) → Ø (d ≤ c) → d < a → False := by
    研磨-linarith（分割：= 0）

  example [LE α] [Std.IsLinearPreorder α]
      (a b c d : α) : a ≤ b → Ø (c ≤ b) → Ø (d ≤ c) → Ø (a ≤ d) → False := by
    研磨-linarith（分割：= 0）
...
* [#10690](https://github.com/leanprover/lean4/pull/10690) adds the `instantiate`, `show_true`, `show_false`,
  ...
  example (as bs cs : Array α) (v₁ v₂ : α)
    ...
    研磨=>
      实例化
      -- 使用 `generation > 0` 显示断言事实
      show_asserted gen > 0
      -- 显示已知为 `True` 的命题，其中包含 `j` 和 `generation > 0`
      show_true j && gen > 0
      -- 显示包含 `as` 或 `bs` 的术语的等价类
      显示_eqcs 为 ||废话
      实例化
  ```
...
* [#10706](https://github.com/leanprover/lean4/pull/10706) adds the `have` tactic for the `grind` interactive mode.
  ...
  example {a b c d e : Nat}
      : a > 0 → b > 0 → 2*c + e <= 2 → e = d + 1 → a*b + 2 > 2*c + d := 通过
    研磨=>
      有：a*b > 0 := Nat.mul_pos h h_1
      利亚
  ```
