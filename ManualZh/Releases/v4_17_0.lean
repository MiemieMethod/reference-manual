...
tagged with this attribute:
  ```lean
  getLast?_eq_some_iff {xs : List α} {a : α} : xs.getLast? = 一些 a ↔ ∃ ys, xs = ys ++ [a]
  ```
...
tactic. The following example works now:
  ...
  example : f x > 1 := by
    磨[
        f, -- 指示 `grind` 使用 `f` 方程定理，
        C -- 指示 `grind` 对 `C` 类型的自由变量进行大小写分割
    ]
  ```
