...
* [#4301](https://github.com/leanprover/lean4/pull/4301)
  ...
  ```
  警告：局部变量“nil”类似于构造函数“List.nil” - 写入“.nil”（带点）或“List.nil”以使用构造函数。
  注意：可以使用 `set_option linter.constructorNameAsVariable false` 禁用此 linter
  ```

  and error:

  ```
  预计标有“[match_pattern]”的模式、构造函数或常量无效
...
* [#4495](https://github.com/leanprover/lean4/pull/4495), [#4692](https://github.com/leanprover/lean4/pull/4692), [#4849](https://github.com/leanprover/lean4/pull/4849)
  ...
  ```lean
  需要 <scope> / <pkg-name> [@ git <rev>]
  -- 示例：
  需要“leanprover”/“doc-gen4”
  需要“leanprover-community”/“proofwidgets”@git“v0.0.39”
  ```
