...
单个包可能包含多个库或可执行程序。
包由一个目录组成，其中包含 {tech (key := "package configuration")}[包配置] 文件和源代码。
软件包可能 {deftech}_require_ 其他软件包，在这种情况下，这些软件包的代码（更具体地说，它们的 {tech (key := "targets")}[目标]）可用。
包的 {deftech (key := "direct dependencies")}_direct dependency_ 是它需要的依赖项，{deftech (key := "transitive dependencies")}_transitive dependency_ 是包的直接依赖项及其传递依赖项。
包可以从 [Reservoir](https://reservoir.lean-lang.org/){TODO}[参考章节]、Lean 包存储库或从手动指定的位置获取。
{deftech (key := "Git dependencies")}_Git dependency_ 由 Git 存储库 URL 以及修订（分支、标签或哈希）指定，并且必须在构建之前在本地克隆，而本地 {deftech (key := "path dependencies")}_path dependency_ 由相对于包目录的路径指定。

:::paragraph
{deftech}_workspace_ 是磁盘上的一个目录，其中包含 {tech}[package] 源代码的工作副本以及所有未指定为本地路径的 {tech (key := "transitive dependencies")}[transitive dependency] 的源代码。
为其创建工作区的包是 {deftech}_root package_。
该工作区还包含该包的任何构建的 {tech (key := "artifacts")}[工件]，从而启用 {tech (key := "incremental builds")}[增量构建]。
 ...
 * `.lake/`：由Lake管理的中间状态，例如构建的{tech}[artifacts]和依赖源代码。
   ...
   * `.lake/packages/`：工作区的 {deftech (key := "package directory")}_package 目录_，其中包含根包的所有非本地传递依赖项的副本，其构建的工件位于其自己的 `.lake` 目录中。
   * `.lake/build/`：{deftech (key := "build directory")}_build 目录_，其中包含根包的构建工件：
     * `.lake/build/bin`：包的 {deftech (key := "binary directory")}_binary 目录_，其中包含构建的可执行文件。
     * `.lake/build/lib`：包的_库目录_，其中包含构建的库和 {tech (key := ".olean files")}[`.olean` 文件]。
...
:::paragraph
{deftech (key := "package configuration")}_package 配置_ 文件指定包的依赖项、设置和目标。
...
清单和 {tech (key := "package configuration")}[程序包配置] 文件一起为程序包指定一组唯一的传递依赖项。
...
持久构建输出（例如目标代码、可执行二进制文件或 {tech (key := ".olean file")}[`.olean` 文件]）称为 {deftech}_artifact_。
在生成工件的过程中，Lake可能需要生成更多工件；例如，将 Lean 程序编译为可执行文件需要将其及其依赖项编译为目标文件，这些目标文件本身是从 C 源文件生成的，而 C 源文件是通过详细说明 Lean 源文件并生成 {tech (key := ".olean files")}[`.olean` 文件] 产生的。
该链中的每个链接都是一个目标，Lake 安排每个链接依次构建。
链的开头是 {deftech (key := "initial targets")}_初始目标_：
 * {tech}_Packages_ 是作为一个单元分发的 Lean 代码单元。
 * {deftech}_Libraries_ 是 Lean {tech (key := "module")}[模块] 的集合，在一个或多个 {deftech (key := "module roots")}_module root_ 下分层组织。
 * {deftech}_Executables_ 由定义 `main` 的_单个_模块组成。
 * {deftech (key := "External libraries")}_外部库_ 是非 Lean *静态* 库，它们将链接到包及其依赖项的二进制文件，包括它们的共享库和可执行文件。
 * {deftech (key := "Custom targets")}_自定义目标_包含运行构建的任意代码，使用 Lake 的内部 API 编写。

除了 Lean 代码之外，包、库和可执行文件还包含影响后续构建步骤的配置设置。
包可以指定一组 {deftech (key := "default targets")}_default 目标_。
默认目标是包中的初始目标，这些目标将在指定了包但未指定特定目标的上下文中构建。
:::

:::paragraph
{deftech}_log_ 包含构建期间生成的信息。
日志被保存，以便可以在 {tech (key := "incremental builds")}[增量构建]期间重播。
日志中的消息有四个级别，按严重性排序：
...
## 包覆盖
%%%
file := "Package--Overrides"
tag := "package-overrides"
%%%

{tech (key := "package configuration")}[程序包配置] 和 {tech (key := "manifest")}[清单] 一起描述了 Lake 期望获取依赖项的确切方式。
...
Lake 的 {deftech}_package overrides_ 允许将包依赖项从一个源重定向到另一个源，而无需修改任何 {tech (key := "package configurations")}[包配置] 或 {tech (key := "manifests")}[清单]。
它们不允许向 {tech (key := "workspace")}[工作区] 添加或删除包。
工作区中的所有传递依赖项都遵循重定向。
...
确定包条目必要语法的一种方法是将临时依赖项添加到与所需配置匹配的 {tech (key := "package configuration")}[包配置]，运行 {lake}`update` 以生成具有该依赖项的清单，然后将条目从清单复制到包覆盖文件中。
...
%%%
file := "Builds"
tag := "zh-buildtools-lake-h003"
%%%

:::paragraph
生成所需的 {tech (key := "artifact")}[工件]，例如 {tech (key := ".olean file")}[`.olean` 文件] 或可执行二进制文件，称为 {deftech}_build_。
构建由 {lake}`build` 命令或需要存在工件的其他命令（例如 {lake}`exe`）触发。
构建由以下步骤组成：

: {deftech (key := "configure package")}[配置]包

  如果 {tech (key := "package configuration")}[程序包配置] 文件比缓存的配置文件 `lakefile.olean` 新，则重新详细说明程序包配置。
  当缓存文件丢失或提供 {lakeOpt}`--reconfigure` 或 {lakeOpt}`-R` 标志时，也会发生这种情况。
  使用 {lakeOpt}`-K` 更改选项不会触发配置文件的重新精化；在这些情况下 {lakeOpt}`-R` 是必要的。

: 计算依赖关系

  确定产生所需输出所需的工件集，以及产生它们的 {tech (key := "targets")}[目标] 和 {tech (key := "facets")}[方面]。
  这个过程是递归的，结果是依赖关系图。
...
: 重放痕迹
  ...
  Lake 使用保存的 {deftech (key := "trace files")}_trace 文件_ 来确定需要构建哪些工件，而不是从头开始重建依赖关系图中的所有内容。
  在构建期间，Lake 记录用于生成每个工件的源文件或其他工件，保存每个输入的哈希值；这些 {deftech}_traces_ 保存在 {tech (key := "build directory")}[构建目录]中。{margin}[更具体地说，每个工件的跟踪文件包含其输入哈希的 Merkle 树哈希混合。]
  如果输入全部未修改，则不会重建相应的工件。
  跟踪文件还记录每个构建任务的 {tech}[log]；这些输出会被重播，就好像工件是重新构建的一样。
  尽可能重用以前的构建产品称为 {deftech (key := "incremental build")}_增量构建_。
 ...
 * {deftech (key := "source directory")}_source 目录_包含可导入的 Lean 源代码。
 * {deftech (key := "library directories")}_library 目录_包含 {tech (key := ".olean files")}[`.olean` 文件] 以及可用于链接的共享库和静态库；它通常由 {tech (key := "root package")}[根包] 的库目录（在 `.lake/build/lib` 中找到）、工作区中其他包的库目录、当前 Lean 工具链的库目录和系统库目录组成。
...
每种类型的初始目标都有相应的默认方面（例如，从可执行目标生成可执行二进制文件或构建包的 {tech (key := "default targets")}[默认目标]）；其他方面可以在 {tech (key := "package configuration")}[软件包配置] 中或通过 Lake 的 {ref "lake-cli"}[命令行接口] 显式请求。
...
: `deps`
  ...
  该包的 {tech (key := "direct dependencies")}[直接依赖项]。

: `transDeps`

  包的 {tech (key := "transitive dependencies")}[传递依赖项]，按拓扑排序。
...
: `olean`
 ...
 模块的 {tech (key := ".olean file")}[`.olean` 文件]。 {TODO}[模块系统完全落地后，添加`olean.private`和`olean.server`的文档]
...
: `transImports`
  ...
  Lean 模块的传递导入，如 {tech (key := ".olean files")}[`.olean` 文件]。
...
Lake {tech (key := "package configuration")}[包配置]文件可能包含 {deftech (key := "Lake scripts")}_Lake 脚本_，它们是可以从命令行执行的嵌入式程序。
...
## 测试和 Lint 驱动程序
%%%
file := "Test-and-Lint-Drivers"
tag := "test-lint-drivers"
%%%

{deftech (key := "test driver")}_测试驱动程序_运行包的测试。
...
{deftech (key := "lint driver")}_lint 驱动程序_ 与之类似，但它由 {lake}`lint` 运行，并检查包是否存在风格问题和其他并非错误、但表明可能存在问题的问题。
Lint 驱动程序只能是可执行文件或脚本，不能是库。

### 配置测试驱动程序
%%%
file := "Configuring-a-Test-Driver"
...
每个 {tech (key := "toolchain")}[工具链] 都有自己的缓存，因为中间构建产品在工具链版本之间不兼容。
但是，工具链的缓存在使用它的所有本地 {tech (key := "workspaces")}[工作空间] 之间共享，因此不需要重建公共依赖项。
...
它在各个源文件、{tech (key := ".olean files")}[`.olean` 文件] 和目标代码级别（而不是整个包级别）跟踪构建产品。

### 映射
%%%
file := "Mappings"
tag := "zh-buildtools-lake-h015"
%%%

当传递 `-o` 选项时，{lake}`build` 跟踪用于生成每个构建产品的输入。
这些以 JSON 行格式存储到 {deftech}_mappings file_ 中，其中文件的每一行必须是有效的 JSON 对象。
映射文件跟踪单个构建，并包括工作区的 {tech (key := "root package")}[根包] 的所有中间和最终构建产品，但不包括其依赖项。
这包括已经是最新且未重新生成的构建产品。
...
## 访问环境
%%%
file := "Accessing-the-Environment"
tag := "zh-buildtools-lake-h018"
%%%
...
### 搜索路径助手
%%%
file := "Search-Path-Helpers"
tag := "zh-buildtools-lake-h019"
%%%
...
### Elan 安装助手
%%%
file := "Elan-Install-Helpers"
tag := "zh-buildtools-lake-h020"
%%%
...
### Lean 安装助手
%%%
file := "Lean-Install-Helpers"
tag := "zh-buildtools-lake-h021"
%%%
...
### Lake 安装助手
%%%
file := "Lake-Install-Helpers"
tag := "zh-buildtools-lake-h022"
%%%
...
## 访问工作区
%%%
file := "Accessing-the-Workspace"
tag := "zh-buildtools-lake-h023"
%%%
