...
精化有多种：命令精化实现每个顶级命令的效果（例如声明 {tech (key := "inductive types")}[归纳类型]、保存定义、求值表达式），而术语精化负责构造在许多命令中出现的术语（例如签名中的类型、定义的右侧或要计算的表达式）。
策略执行是术语精化的专业化。
...
当前 {tech (key := "section scope")}[节范围] 中的开放命名空间也会影响使用哪些解析规则，因为仅当给定命名空间打开时，解析器扩展才可以设置为活动状态。

当遇到歧义时，选择最长的匹配解析。
如果没有唯一的最长匹配，则两个匹配解析都保存在语法树中的 {deftech (key := "choice node")}[选择节点] 中，稍后由精化器解析。
...
# 宏展开和精化
%%%
file := "Macro-Expansion-and-Elaboration"
tag := "macro-and-elab"
%%%
...
Lean {deftech (key := "environments")}[环境] 包含从名称到定义的常见映射以及 {deftech (key := "environment extensions")}[环境扩展] 中定义的附加数据，这些数据是与环境关联的附加表；环境扩展用于跟踪有关 Lean 代码的大多数其他信息，包括 {tactic}`simp` 引理、自定义漂亮打印机以及编译器的中间表示等内部结构。
命令精化还维护一个消息日志，其中包含编译器的信息输出、警告和错误的内容、一组将元数据与原始语法关联起来的 {tech (key := "info trees")}[信息树]（用于交互功能，如显示证明状态、标识符完成和显示文档）、累积的调试跟踪、打开的 {tech (key := "section scopes")}[节范围] 以及与宏展开。
 ...
 * {tech (key := "impredicative")}[必然]，定义上证明无关，{tech (key := "universe")}[命题]的扩展 {tech (key := "propositions")}[宇宙]
 * {tech (key := "predicative")}[谓词]，数据域的非累积层次结构
...
到递归器的转换分两个阶段进行：在精化期间，模式匹配的使用被对 {deftech (key := "auxiliary matching functions")}_辅助匹配函数_（也称为 {deftech (key := "matcher functions")}_matcher 函数_）的调用所取代，这些函数实现了代码中出现的特定大小写区分。
...
仍包含递归但已详细精化为核心语言的定义称为 {deftech (key := "pre-definition")}[预定义]。
...
使用类型的递归器无法捕获的其他递归模式的函数使用 {tech (key := "well-founded recursion")}[良基递归] 进行转换，这是结构递归，证明某些 {tech}[measure] 在每次递归调用时都会减少，或者使用 {ref "partial-fixpoint"}[partial fixpoints]，逻辑上捕获通过诉诸领域理论构造来至少部分地描述函数的规范。
Lean 可以自动导出许多终止证明，但有些需要手动证明。
良基递归更灵活，但由于证明项表明测度减少，并且它们的定义方程可能仅在命题上成立，因此生成的函数在内核中执行速度通常较慢。
为了向通过结构和良基递归定义的函数提供统一的接口并检查其自身的正确性，精化器证明了将函数与其原始定义相关联的 {deftech (key := "equational lemmas")}[方程引理]。
...
详细说明模块后，使用内核检查对环境的每个添加，模块对全局环境（包括扩展）所做的更改将序列化到 {deftech (key := ".olean file")}[`.olean` 文件]。
...
启动之前，精化器必须正确初始化。
Lean 本身包含 {deftech (key := "initialization")}[初始化] 代码，必须运行该代码才能正确构造编译器的初始状态；此代码在加载任何模块之前以及调用精化器之前运行。
