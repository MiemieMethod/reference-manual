...
除了绑定变量之外，术语语言中的变量还可以指 {tech (key := "constructors")}[构造函数]、{tech (key := "type constructors")}[类型构造函数]、{tech (key := "recursors")}[递归函数]、{deftech (key := "defined constants")}[定义的常量] 或不透明常量。
...
除了具有类型之外，术语还通过 {deftech (key := "definitional equality")}_定义等价_相关。
...
定义等价 包括 {deftech}[reduction] 的以下形式：
 ...
 : {deftech}[δ]（增量）
    ...
    将出现的 {tech (key := "defined constants")}[已定义常量] 替换为定义的值

 : {deftech}[ι] (iota)

    减少目标是构造函数的递归器（原始递归）

 : {deftech (key := "ζ")}[z] (zeta)
...
已进行所有可能减少的项采用 {deftech (key := "normal form")}_正常形式_。
...
export S (f1 f2)
```
定义等价 包括函数 {deftech (key := "η-equivalence")}[η-等价] 和单构造函数归纳类型。
也就是说，如果 {lean}`S` 是具有字段 {lean}`f1` 和 {lean}`f2` 的结构，则 {lean}`fun x => f x` 定义上等于 {lean}`f`，{lean}`S.mk x.f1 x.f2` 定义上等于 {lean}`x`。
它还具有 {deftech (key := "proof irrelevance")}_证明无关性_：同一命题的任何两个证明在定义上都是相等的。
它是自反且对称的，但不具有传递性。
::::
...
Lean 中的基本类型为 {tech}[universes]、{tech}[function] 类型、{name}`Quot` 的商、{tech (key := "type constructors")}[归纳类型] 的 {tech (key := "inductive types")}[类型构造函数]。
{tech (key := "Defined constants")}[定义的常量]、{tech (key := "recursors")}[递归器]的应用、函数应用、{tech (key := "axioms")}[公理]或 {tech (key := "opaque constants")}[不透明常量] 还可以给出类型，就像它们可以产生任何其他类型的项一样。
...
{deftech (key := "Propositions")}[命题] 是接受证据的有意义的陈述。 {index}[提议]
无意义的陈述不是命题，但错误的陈述才是。
所有命题均按 {lean}`Prop` 分类。
...
Lean 的 Universe 不是 {deftech (key := "cumulative")}[累积]；{index}[累积性] `Type u` 中的类型不会自动出现在 `Type (u + 1)` 中。
每种类型都恰好栖息在同一个宇宙中。
...
Lean 支持 {deftech (key := "universe polymorphism")}_universe 多态性_、{index (subterm := "universe")}[多态性] {index}[universe 多态性]，这意味着 Lean 环境中定义的常量可以采用 {deftech (key := "universe parameters")}[universe 参数]。
...
`imax` 用于实现 {lean}`Prop` 的 {tech (key := "impredicative")}[指示] 量化。
...
除了使用 `autoImplicit` 之外，还可以使用 `universe` 命令将特定标识符声明为特定 {tech (key := "section scope")}[节范围] 中的 Universe 变量。
...
当一种类型的 Universe 小于某些上下文中预期的 Universe 时，{deftech (key := "universe lifting")}_universe lift_ 运算符可以弥补这一差距。
这些是给定类型术语的包装器，它们位于比包装类型更大的宇宙中。
起重操作员有两名：
 * {name}`PLift` 可以将任何类型（包括 {tech (key := "propositions")}[命题]）提升一级。它可用于在数据结构（例如列表）中包含证明。
