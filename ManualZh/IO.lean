...
Lean 是一种纯函数式编程语言。
虽然 Lean 代码在运行时严格求值，但类型检查期间（尤其是检查 {tech}[定义等价] 时）使用的求值顺序在形式上未指定，并且使用了许多启发式方法来提高性能，但可能会发生变化。
...
它可以被认为是一个 {tech}[状态单子]，其中状态就是整个世界。
...
{lean}`IO` monad 中的错误处理使用与任何其他 {tech}[异常 monad] 相同的设施。
