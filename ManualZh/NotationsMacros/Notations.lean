...
符号与 {tech (key := "section scopes")}[节范围] 的交互方式与属性和运算符相同。
默认情况下，符号在任何可传递导入建立符号的模块中都可用，但它们可以声明为 `scoped` 或 `local`，以分别将其可用性限制在当前名称空间已打开的上下文或当前 {tech (key := "section scope")}[节范围] 中。

与运算符一样，解析符号时使用 {tech (key := "local longest-match rule")}[本地最长匹配规则]。
...
当匹配函数应用术语时，新符号将显示在 {tech (key := "proof states")}[证明状态]、错误消息和 Lean 的其他输出中，否则会显示。
