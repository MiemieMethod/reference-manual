...
度量的类型必须配备 {tech (key := "well-founded relation")}[有充分基础的关系]，它确定度量减少意味着什么。
...
如果不存在无限下降链，则关系 `≺` 是 {deftech (key := "well-founded relation")}_有充分基础的关系_
...
* 作为 {name}`SizeOf` 类型类实例（提供方法 {name}`SizeOf.sizeOf`）的每个类型都具有良好基础的关系。
  对于这些类型，{lean}`x₁ ≺ x₂` 当且仅当 {lean}`sizeOf x₁ < sizeOf x₂`。对于 {tech (key := "inductive types")}[归纳类型]，{lean}`SizeOf` 实例由 Lean 自动派生。
...
为了推断合适的终止 {tech}[measure]，Lean 考虑多个 {deftech (key := "basic termination measures")}_basic TerminationMeasures_（它们是类型 {name}`Nat` 的终止测量），然后尝试这些测量的所有元组。
...
{deftech (key := "Candidate measures")}_候选度量_是基本度量或基本度量的元组。
...
为了避免尝试所有度量元组的组合爆炸，Lean 首先将所有 {tech (key := "basic termination measures")}[基本终止度量] 制成表格，确定基本度量是递减、严格递减还是非递减。
...
Lean 支持使用 {tech (key := "well-founded recursion")}[良基递归] 定义 {tech (key := "mutually recursive")}[互递归]函数。
...
1.  Lean 使用 {name}`wfParam` {tech}[gadget] 注释函数参数或参数子项的出现。
    ...
    {name}`wfParam` 设备也从 {tech (key := "projection function")}[投影功能] 应用程序中浮出。

2.  带注释的函数体使用 {ref "the-simplifier"}[简化器] 进行简化，仅使用 {attr}`wf_preprocess` {tech (key := "custom simp set")}[自定义 simp 集] 中的简化规则。
...
本节通过 {tech (key := "well-founded recursion")}[良基递归] 非常简要地介绍了终止证明背后的数学结构，这些数学结构有时可能会出现。
