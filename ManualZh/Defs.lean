...
所有这些命令都会导致 Lean 变为 {tech (key := "elaborator") -normalize}[详细精化]基于 {tech (key := "signature")}[签名]的术语。
...
## 带括号的参数绑定
%%%
file := "Bracketed-Parameter-Bindings"
...
带 `:=` 的括号参数为参数分配默认值。
具有默认值的参数称为 {deftech (key := "optional parameters")}_可选参数_。
在调用站点，如果未提供参数，则使用提供的术语来填充它。
签名中的先前参数在默认值范围内，并且它们在调用站点的值将替换为默认值项。

如果提供了 {ref "tactics"}[策略脚本]，则在调用站点执行策略以合成参数值；通过策略填写的参数称为{deftech (key := "automatic parameters")}_自动参数_。
```grammar
($x $x* : $t := $e)
```
:::
...
大括号中的参数表示 {tech (key := "implicit")}[隐式] 参数。
...
双花括号中的参数表示 {tech (key := "strict implicit")}[严格隐式] 参数。
`⦃ … ⦄` 和 `{{ … }}` 等效。
...
方括号中的参数表示 {tech (key := "instance implicit")}[实例隐式] 参数，这些参数是使用 {tech (key := "synthesis")}[实例合成] 在调用站点合成的。
```grammar
[$[$x :]? $t]
```
:::
...
{ref "function-application"}[函数应用]部分详细介绍了 {tech (key := "optional parameter")}[可选]、{tech (key := "automatic parameter")}[自动]、{tech (key := "implicit")}[隐式]和 {tech (key := "instance implicit")}[实例隐式]参数的解释。
...
与签名中写入的名称不对应的隐式参数被分配的名称类似于证明中 {tech (key := "inaccessible")}[不可访问] 假设的名称，无法引用。
它们出现在带有匕首的签名中（`'✝'`）。
这可以防止 Lean 任意选择的名称通过用作 {tech (key := "named argument")}[命名参数] 成为 API 的一部分。
...
由于 {tech (key := "section variables")}[节变量]，在插入参数后会发生自动隐式参数插入。
...
在精化器中，此替换由常数的 {tech (key := "reducibility")}[约简性] 控制。
...
但是，{tech (key := "instance implicit")}[实例隐式]参数必须在标头中指定或指定为 {tech (key := "section variables")}[节变量]。
...
{keyword}`def` 的定义是 {tech (key := "semireducible")}[半可约]。
...
{deftech (key := "Abbreviations")}[缩写] 与 {keyword}`def` 的定义相同，但它们是 {tech (key := "reducible")}[可约]。
...
{deftech (key := "Opaque constants")}_不透明常量_是不受内核中的 {tech (key := "δ")}[δ-约简] 影响的已定义常量。
它们对于指定某些函数的存在很有用。
...
:::paragraph
由于 {tech (key := "propositions")}[命题] 是其居民视为证明的类型，因此 {deftech (key := "theorems")}[定理] 和定义在技术上非常相似。
...
* 默认定理为 {tech (key := "irreducible")}[不可约]。
