...
def main :=
  manualMain (%doc ManualZh) (config := config) (extraSteps := [extractExamples])
where
  config := {
    ...
    sourceLink := some "https://github.com/MiemieMethod/reference-manual",
    issueLink := some "https://github.com/MiemieMethod/reference-manual/issues",
    searchPriorities := {
      domains := .ofList [
